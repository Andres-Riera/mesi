...
class BigUnsigned : public BigNumber<Base> {
 public:
  ...
  virtual operator BigUnsigned<Base>() const override { return *this; }
  virtual operator BigInteger<Base>() const override { return BigInteger<Base>(*this); }
  virtual operator BigRational<Base>() const override { 
    BigInteger<Base> num(*this);
  ...
  virtual BigNumber<Base>& add(const BigNumber<Base>& number) const override {
    const BigUnsigned<Base> other = number;
    auto* result = new BigUnsigned<Base>(*this + other);
    return *result;
  }
...
BigUnsigned<Base>::BigUnsigned(const unsigned char* number) {
  ...
  for (char c : number_str) {
    ...
    if (c >= '0' && c <= '9') {
      ...
      throw BigNumberBadDigit(std::string(1, c));
    }
    if (digit_value >= Base) {
      throw BigNumberBadDigit(std::string(1, c));
    }
